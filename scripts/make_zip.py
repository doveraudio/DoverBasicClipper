...
def  main():
  ...
  installer = "\installer\DoverBasicClipper Installer.exe"
   
  if demo:
    installer = "\installer\DoverBasicClipper Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\DoverBasicClipper manual.pdf" 
  ]
  ...
  ZIPNAME = "DoverBasicClipper-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "DoverBasicClipper-v" + FULLVERSIONSTR + "-win-demo.zip"
